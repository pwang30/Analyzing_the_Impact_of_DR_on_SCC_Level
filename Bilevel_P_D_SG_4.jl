...
for t in 1:T                                                                                                  # dual constraints for generation, when t<=T-1, assume SGs in bus 2 are strategic
    ...
    @constraint(model, Oᵐ[4]*b[t] -λᴱ[t] +μᵐᵃˣˢᴳ⁴[t] -μᵐⁱⁿˢᴳ⁴[t] >=0)                
end
...
for t in 1:T
    @constraint(model, Mc_1[t]>= λᴱ[t]*Pˢᴳₘᵢₙ[4])
    @constraint(model, Mc_1[t]>= λᴱ[t]*Pˢᴳₘₐₓ[4] +bᵐᵃˣ*Oᵐ[4]*Pˢᴳ⁴[t] -bᵐᵃˣ*Oᵐ[4]*Pˢᴳₘₐₓ[4])
    @constraint(model, Mc_1[t]<= λᴱ[t]*Pˢᴳₘᵢₙ[4]  +bᵐᵃˣ*Oᵐ[4]*Pˢᴳ⁴[t] -bᵐᵃˣ*Oᵐ[4]*Pˢᴳₘᵢₙ[4])
    @constraint(model, Mc_1[t]<= λᴱ[t]*Pˢᴳₘₐₓ[4] )
end
revenue_energy_UL=sum( Mc_1 )                      # revenue from market clearing
cost_nl_UL=sum(Oⁿˡ[4].*yˢᴳ⁴ )                  # no-load cost of strategic SGs
cost_gene_UL=sum(Oᵐ[4].*Pˢᴳ⁴)                   # generation cost of strategic SGs
...
for t in 1:T
    @constraint(model, Mc_2[t]>= b[t]*Pˢᴳₘᵢₙ[4] +Pˢᴳ⁴[t] -Pˢᴳₘᵢₙ[4])
    @constraint(model, Mc_2[t]>= b[t]*Pˢᴳₘₐₓ[4] +bᵐᵃˣ*Pˢᴳ⁴[t] -bᵐᵃˣ*Pˢᴳₘₐₓ[4])
    @constraint(model, Mc_2[t]<= b[t]*Pˢᴳₘᵢₙ[4] +bᵐᵃˣ*Pˢᴳ⁴[t] -bᵐᵃˣ*Pˢᴳₘᵢₙ[4])
    @constraint(model, Mc_2[t]<= b[t]*Pˢᴳₘₐₓ[4] +Pˢᴳ⁴[t] -Pˢᴳₘₐₓ[4])
end
...
cost_gene_LL=sum(Oᵐ[1]*Pˢᴳ¹) +sum(Oᵐ[2].*Pˢᴳ²) +sum(Oᵐ[3].*Pˢᴳ³) +sum(Oᵐ[4].*Mc_2)  
