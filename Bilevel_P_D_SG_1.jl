...
for t in 1:T                                                                                                  # dual constraints for generation, when t<=T-1, assume SGs in bus 2 are strategic
    @constraint(model, Oᵐ[1]*b[t] -λᴱ[t] +μᵐᵃˣˢᴳ¹[t] -μᵐⁱⁿˢᴳ¹[t] >=0)                
...
for t in 1:T
    @constraint(model, Mc_1[t]>= λᴱ[t]*Pˢᴳₘᵢₙ[1])
    @constraint(model, Mc_1[t]>= λᴱ[t]*Pˢᴳₘₐₓ[1] +bᵐᵃˣ*Oᵐ[1]*Pˢᴳ¹[t] -bᵐᵃˣ*Oᵐ[1]*Pˢᴳₘₐₓ[1])
    @constraint(model, Mc_1[t]<= λᴱ[t]*Pˢᴳₘᵢₙ[1]  +bᵐᵃˣ*Oᵐ[1]*Pˢᴳ¹[t] -bᵐᵃˣ*Oᵐ[1]*Pˢᴳₘᵢₙ[1])
    @constraint(model, Mc_1[t]<= λᴱ[t]*Pˢᴳₘₐₓ[1] )
end
revenue_energy_UL=sum( Mc_1 )                      # revenue from market clearing
cost_nl_UL=sum(Oⁿˡ[1].*yˢᴳ¹ )                  # no-load cost of strategic SGs
cost_gene_UL=sum(Oᵐ[1].*Pˢᴳ¹)                   # generation cost of strategic SGs 
...
for t in 1:T
    @constraint(model, Mc_2[t]>= b[t]*Pˢᴳₘᵢₙ[1] +Pˢᴳ¹[t] -Pˢᴳₘᵢₙ[1])
    @constraint(model, Mc_2[t]>= b[t]*Pˢᴳₘₐₓ[1] +bᵐᵃˣ*Pˢᴳ¹[t] -bᵐᵃˣ*Pˢᴳₘₐₓ[1])
    @constraint(model, Mc_2[t]<= b[t]*Pˢᴳₘᵢₙ[1] +bᵐᵃˣ*Pˢᴳ¹[t] -bᵐᵃˣ*Pˢᴳₘᵢₙ[1])
    @constraint(model, Mc_2[t]<= b[t]*Pˢᴳₘₐₓ[1] +Pˢᴳ¹[t] -Pˢᴳₘₐₓ[1])
end
...
cost_gene_LL=sum(Oᵐ[1]*Mc_2) +sum(Oᵐ[2].*Pˢᴳ²) +sum(Oᵐ[3].*Pˢᴳ³) +sum(Oᵐ[4].*Pˢᴳ⁴)  
