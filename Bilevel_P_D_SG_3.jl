...
for t in 1:T                                                                                                  # dual constraints for generation, when t<=T-1, assume SGs in bus 2 are strategic
    ...
    @constraint(model, Oᵐ[2] -λᴱ[t] +μᵐᵃˣˢᴳ²[t] -μᵐⁱⁿˢᴳ²[t] >=0)                
    @constraint(model, Oᵐ[3]*b[t] -λᴱ[t] +μᵐᵃˣˢᴳ³[t] -μᵐⁱⁿˢᴳ³[t] >=0)
...
for t in 1:T
    @constraint(model, Mc_1[t]>= λᴱ[t]*Pˢᴳₘᵢₙ[3])
    @constraint(model, Mc_1[t]>= λᴱ[t]*Pˢᴳₘₐₓ[3] +bᵐᵃˣ*Oᵐ[3]*Pˢᴳ³[t] -bᵐᵃˣ*Oᵐ[3]*Pˢᴳₘₐₓ[3])
    @constraint(model, Mc_1[t]<= λᴱ[t]*Pˢᴳₘᵢₙ[3] +bᵐᵃˣ*Oᵐ[3]*Pˢᴳ³[t] -bᵐᵃˣ*Oᵐ[3]*Pˢᴳₘᵢₙ[3])
    @constraint(model, Mc_1[t]<= λᴱ[t]*Pˢᴳₘₐₓ[3] )
end
revenue_energy_UL=sum( Mc_1 )                      # revenue from market clearing
cost_nl_UL=sum(Oⁿˡ[3].*yˢᴳ³ )                  # no-load cost of strategic SGs
cost_gene_UL=sum(Oᵐ[3].*Pˢᴳ³)                   # generation cost of strategic SGs 
...
for t in 1:T
    @constraint(model, Mc_2[t]>= b[t]*Pˢᴳₘᵢₙ[3] +Pˢᴳ³[t] -Pˢᴳₘᵢₙ[3])
    @constraint(model, Mc_2[t]>= b[t]*Pˢᴳₘₐₓ[3] +bᵐᵃˣ*Pˢᴳ³[t] -bᵐᵃˣ*Pˢᴳₘₐₓ[3])
    @constraint(model, Mc_2[t]<= b[t]*Pˢᴳₘᵢₙ[3] +bᵐᵃˣ*Pˢᴳ³[t] -bᵐᵃˣ*Pˢᴳₘᵢₙ[3])
    @constraint(model, Mc_2[t]<= b[t]*Pˢᴳₘₐₓ[3] +Pˢᴳ³[t] -Pˢᴳₘₐₓ[3])
end
...
cost_gene_LL=sum(Oᵐ[1]*Pˢᴳ¹) +sum(Oᵐ[2].*Pˢᴳ²) +sum(Oᵐ[3].*Mc_2) +sum(Oᵐ[4].*Pˢᴳ⁴)  
...
sum(RC_1)+sum(RC_2)+sum(RC_3)+sum(RC_4)